...
async function handleIncomeAction(aiData, groupData, user, chatId, txnId, time) {
	...
	if (!memory.wallet) memory.wallet = {
		IDR: 0,
		USD: 0
	};
...
async function handleExpenseAction(aiData, groupData, user, chatId, txnId, time) {
	...
	if (currency === 'USD' && countsToDailyLimit) {
		if (!memory.dailySpent) memory.dailySpent = {
			USD: 0,
			limit: CONFIG.DEFAULT_DAILY_LIMIT,
	...
	if (currency === 'USD') {
		if (!memory.monthlySpent) memory.monthlySpent = {
			USD: 0,
			limit: CONFIG.DEFAULT_MONTHLY_LIMIT,
...
function handleMonthlyInfo(groupData) {
	const memory = groupData.memory;
	const monthly = memory.monthlySpent || {
		USD: 0,
		limit: CONFIG.DEFAULT_MONTHLY_LIMIT,
		month: moment().format('YYYY-MM'),
		categories: {}
...
async function approveTransaction(chatId, args, groupData, userId) {
	...
	if (txn.type === 'income' && !txn.canceled) {
		if (!groupData.memory.wallet) groupData.memory.wallet = {
			IDR: 0,
			USD: 0
		};
	...
	const keyboard = {
		reply_markup: {
			inline_keyboard: [
				[{
						text: '📋 List Groups',
						callback_data: 'super_listgroups'
					},
					{
						text: '⚙️ Global Config',
						callback_data: 'super_config'
					}
				],
				[{
						text: '📊 Statistics',
						callback_data: 'super_stats'
					},
					{
						text: '🔧 Tools',
						callback_data: 'super_tools'
					}
				],
				[{
						text: '🔄 Refresh',
						callback_data: 'super_refresh'
					},
					{
						text: '❌ Close',
						callback_data: 'super_close'
					}
				]
			]
		}
...
async function showAllGroupsPanel(chatId) {
	...
	groups.sort(([, a], [, b]) => new Date(b.created) - new Date(a.created))
		...
		.forEach(([chatId, data], index) => {
			const wallet = data.memory.wallet || {
				IDR: 0,
				USD: 0
			};
...
async function showGroupStatus(params, chatId) {
	...
	if (groupData) {
		const wallet = groupData.memory.wallet || {
			IDR: 0,
			USD: 0
		};
		const rate = groupData.memory.exchangeRate || CONFIG.DEFAULT_RATE;
...
bot.on('message', async (msg) => {
	...
	try {
		...
		if (!enableChat && !isCommand && !isButtonCommand) {
			...
			log('debug', `Chat feature disabled for non-command messages`, {
